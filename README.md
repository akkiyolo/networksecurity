...
An end-to-end **MLOps pipeline** for detecting phishing websites using machine learning. The project automates everything from data ingestion (MongoDB) through model training, experiment tracking (MLflow / DagsHub), and serving predictions via a **FastAPI** REST API — all containerised with Docker and deployed to AWS via GitHub Actions CI/CD.
...
The pipeline (`.github/workflows/main.yml`) runs on every push to `main` (excluding `README.md` changes):
